...
export default function RegisterScheduleAdmin(props) {
    const { infoAllShiftInDay, shiftList, arrayDayInWeek } = props
                                            ...
                                            return infoAllShiftInDay.map((shiftInDay, inx1) => {
                                                if (shiftInDay.day === day_in_week && shiftInDay.shiftInfo.id === shift.id) {
                                                    return shiftInDay.amount ? <td key={inx1}>
                                                        <DisplayNumberExpert numberOfExpert={shiftInDay.amount} shiftInDayId={shiftInDay.id} dayCurrent={arrayDayInWeek[indx2]} />
                                                    </td> : <td key={inx1} >
                                                        <DisplayEmptyShift />
                                                    </td>
                                                }
